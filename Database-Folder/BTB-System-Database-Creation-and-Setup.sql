...
CREATE EVENT deleteTripDateOnDayPassed
    ON SCHEDULE 
        EVERY 1 DAY
            STARTS select cast(concat(ADDDATE(CURDATE(), INTERVAL 1 DAY), ' ', '00:01:00') as datetime) as dt
    DO 
        DELETE FROM `lnkbooks` WHERE TRIPDATE < NOW()
